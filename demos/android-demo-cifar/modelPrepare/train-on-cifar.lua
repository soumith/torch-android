...
-- add target rate (Karam Park)
cmd:option('-targetRate', 0.7, 'target recognize success rate')
cmd:text()
opt = cmd:parse(arg)
...
--del:read(torch.DiskFile(opt.network))
end
...
function train(dataset)
   ...
   -- save mean & std values for use model on other size without loading Cifat-10 data set again (Karam Park)
   torch.save('mean_u',mean_u)
   torch.save('mean_v',mean_v)
   torch.save('std_u',std_u)
   torch.save('std_v',std_v)
...
function test(dataset)
   ...
   -- add for check target rate (Karam Park)
...
-- Stop loop after satisfying required target rate (Karam Park)
while (rate<opt.targetRate) do
   -- train/test
   train(trainData)
   test(testData)
